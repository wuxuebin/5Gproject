            ...
            if (location.getLocType() == BDLocation.TypeGpsLocation) {
                ...
                for (int i = 0; i < cellInfoList.size(); i++) {
                    ...
                    if (cellInfo instanceof CellInfoNr) {
                        ...
                        if(cellSignalStrengthNr.getSsRsrp()>=-85&&cellSignalStrengthNr.getSsRsrp()<-40){
                            points1.add(ll);
                            OverlayOptions ooPolyline = new PolylineOptions().width(13).color(0xAA2E8B57).points(points1);
                            mPolyline = (Polyline) mBaiduMap.addOverlay(ooPolyline);
                        }
                        else if(cellSignalStrengthNr.getSsRsrp()>=-95&&cellSignalStrengthNr.getSsRsrp()<-85){
                            points2.add(ll);
                            OverlayOptions ooPolyline = new PolylineOptions().width(13).color(0xAA008000).points(points2);
                            mPolyline = (Polyline) mBaiduMap.addOverlay(ooPolyline);
                        }
                        else if(cellSignalStrengthNr.getSsRsrp()>=-105&&cellSignalStrengthNr.getSsRsrp()<-95){
                            points3.add(ll);
                            OverlayOptions ooPolyline = new PolylineOptions().width(13).color(0xAAFFFFE0).points(points3);
                            mPolyline = (Polyline) mBaiduMap.addOverlay(ooPolyline);
                        }
                        else if(cellSignalStrengthNr.getSsRsrp()>=-115&&cellSignalStrengthNr.getSsRsrp()<-105){
                            points4.add(ll);
